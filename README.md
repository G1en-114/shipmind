...
- **Skill 级**：每 Skill 一套 `evals/cases/<skill>.jsonl`，必含正向、负向、边界三类；带/不带 Skill 各跑一遍，差值即贡献，汇总 `evals/BENCHMARK.md`——方法论对齐 NVIDIA Verified Skills 的 Tier3 Evaluated 关卡。负向用例（不该触发时必须不触发）与正向同等重要。
- **确定性模块零容差**：航线 XTE/CPA、日志生成用构造用例精确断言；ML 模块给置信度与 AUC。
- **回归评测**：Step 3.7 Flash 在 API 与 NIM 本地端点间切换必须过同一评测集。
