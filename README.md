...
- **双轨对照**（`evals/BENCHMARK.md`）：机舱声学异常检测同时实现启发式轨（信号特征 z-score，可解释）与 ML 轨（DCASE2020 官方配方自编码器）。真实 MIMII 数据三机种对照——启发式轨 pump 0.856 / valve 0.988 / fan 0.666 全面胜出；并发现 AE 带符号重构误差在 pump 上倒挂（0.260，偏离度口径回到 0.797），证实“异常=重构误差更高”的默认假设在真实数据上不总成立。
