...
深色外观服务于夜间值班环境，保持与浅色模式相同的信息层级和语义色，不另造一套视觉语言。画布为 `#0b0b0d`，面板为 `#17171a`，内层区域为 `#232327`；主要文字使用 `#f5f5f7`，次要文字使用 `#98989d`。操作蓝提升为 `#409cff`，正常、关注和危急分别使用 `#30d158`、`#ff9f0a`、`#ff6961`。雷达、航线和声学 Canvas 必须从 CSS 变量读取配色，并在主题切换后重绘。

外观控制支持跟随系统、浅色和深色三档，选择保存在本机。首屏脚本在样式加载前设置有效主题和浏览器主题色，避免深色环境先闪出浅色画布；系统外观变化仅在“跟随系统”状态下接管界面。
